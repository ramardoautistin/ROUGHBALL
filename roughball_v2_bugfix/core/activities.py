...
def training_tuesday(team, era_id):
    ...
    print("   BASE STATS (Primary):")
    print("   [1] ♣ Rush Tackles (TKL)")
    print("   [2] ♥ Box Snaps (AWR)")
    print("   [3] ♠ Pursuit Tackling (INT)")
    print("   [4] ♦ Shuffle Passing (PAS)")
    print("\n   SAVING THROWS (Secondary):")
    print("   [5] ♣ Scrum Locks (STA)")
    print("   [6] ♥ Carrier Sprints (SPD)")
    print("   [7] ♠ Post Kicking (KCK)")
    print("   [8] ♦ Contested Catching (CAT)")
    ...
    # Map choice to stats (separate base stats from saving throws)
    drill_map = {
        # BASE STATS (apply to primary only)
        '1': ('TKL', None, 'Rush Tackles (TKL)', 'stat'),
        '2': ('AWR', None, 'Box Snaps (AWR)', 'stat'),
        '3': ('INT', None, 'Pursuit Tackling (INT)', 'stat'),
        '4': ('PAS', None, 'Shuffle Passing (PAS)', 'stat'),
        # SAVING THROWS (apply to save only)
        '5': (None, 'STA', 'Scrum Locks (STA)', 'save'),
        '6': (None, 'SPD', 'Carrier Sprints (SPD)', 'save'),
        '7': (None, 'KCK', 'Post Kicking (KCK)', 'save'),
        '8': (None, 'CAT', 'Contested Catching (CAT)', 'save')
    }
    
    primary_stat, save_stat, drill_name, drill_type = drill_map[choice]
    
    # Roll for bonus (d4-1 = 0-3)
    bonus = max(0, roll_d4() - 1)
    
    # Apply bonus based on drill type
    if drill_type == 'stat':
